...
`132a_power_gate_curve.tsv` + `S40_POWER_GATE_SPEC.md`（功效闸门）

> **一句话**：排除名单已经查全了，**没有一个已发表的皮肤黑色素瘤 GWAS 资源在名单之外**；
> 名单外能查证的最大单一队列是 MVP 的 5,364 例组织学确诊侵袭性黑色素瘤，
> 其 N_eff 只有发现集的 **0.43 倍**。§2 的两条取数路径里，第二条（consortium
...
GWAS Catalog 里 47 项 melanoma 研究逐条查过 `cohort` 字段，
**2020 年之后的每一项都带 UKB、FinnGen、QSKIN、Q-MEGA、MIA 或 GERA 标签**。
换句话说：**没有任何已发表的皮肤黑色素瘤 GWAS 可以直接拿来当外部验证集。**
...
读法 A 的通过率在 R ≈ 4–5 达到峰值 94.2%，之后**下降**：
R = 10 时 82.1%、R = 12 时 75.7%、R = 16 时 63.9%（f = 0.50）；
f = 1.00 时 R = 16 只剩 **18.5%**。

读法 B 下更干脆：**g = f·√R 一旦超过约 3，主终点直接归零**——
f = 1.00、R = 10 时中位 494 个显著位点（背景共 655 个），通过率 **0.0%**；
R = 12、16 同样是 0.0%。

原因是结构性的，与读法无关：主终点是"显著位点集"相对"全部可检验位点"的富集。
功效越大，显著集越接近背景，**fold 必然趋向 1**。

**➜ 这也要带回给作者：§6 的第 3 条（fold > 1 且 P < 0.05）在高功效下会自我否定。**
...
|---|---:|---:|---|
| 门槛 | 4.00 | 1.000 | — |
| MVP PheCode 172.1（45,274 例）| 3.29 | 0.907 | ❌ 差 9%，**且表型不合 §2.2** |
| MVP PheCode 172.11（10,468 例）| 0.83 | 0.455 | ❌ |
| MVP 组织学确诊侵袭性（5,364 例）| 0.43 | 0.328 | ❌ |

用 MVP 的对照池（436,283）反解：§2.2 合规的**组织学确诊侵袭性**队列
需要 **≈ 55,600 例**才能达到 R = 4。MVP 现有 5,364 例，**差约 10.4 倍**。

...
**fold 从 4.96 一路衰减到 1.05，但不是单调的**——g = 1.061 时回升到 6.32、
g = 2.121 时回升到 1.27。**Fisher P 大体呈 U 形但带抖动**：
g ≈ 1.41 触底（4.7×10⁻⁶），之后回升，**在 g = 2.828（P = 0.0444）与
g = 3.000（P = 0.0527）之间越过 0.05**。

（抖动本身有解释：位点是离散的，g 每抬一点，一批 novel 位点和一批 known 位点
不会同时越过 FDR 线，谁先谁后就让 fold 上下摆。这也正是中断那次
f = 0.75、R = 6 掉到 85.4% 的原因——那一格 P = 0.034，
离 0.05 已经很近，5% 的覆盖率扰动足以把它推过去。）
...
### 3.3 门槛：全覆盖是 R ≥ 4，但按 §2.4 的合格下限应当取 R ≥ 5
...
**但把覆盖率降到 §2.4 的合格下限 95%，R = 4 那一格就站不住了**
（2,000 次重复，走包的完整路径）：

| R | g | 显著位点 | fold | Fisher P | 通过率 |
|---:|---:|---:|---:|---:|---|
| 3 | 0.866 | 4 | 7.431 | 0.00366 | 0.0% |
| **4** | **1.000** | **8** | 4.954 | 0.00487 | **80.9% ± 0.9** ← **贴着 80% 线** |
| 5 | 1.118 | 17 | 4.662 | 8.4×10⁻⁵ | 100.0% |
| 12 | 1.732 | 250 | 1.364 | 0.0105 | 99.6% ± 0.1 |
| 16 | 2.000 | 318 | 1.256 | 0.0211 | 93.3% ± 0.6 |

R = 4 名义上过线（80.9 ≥ 80），但**落在一个蒙特卡洛标准误之内**，
不能当作稳当的合格判定。理由很直接：g = 1 时中位数**恰好是 8 个位点**，
也就是恰好卡在信息闸门上，丢掉 5% 的变异就足以把它推到 8 以下。

**➜ 稳妥的门槛是 R ≥ 5**：N_eff ≥ **246,685**，对照池极大时约 **61,700 例**，
用 MVP 的对照池（436,283）反解则是 **71,823 例**。
...
需要 **55,627 例**才能达到 R = 4（对照池无限时 49,339 例）；
按 §3.3 稳妥的 R ≥ 5 则需要 **71,823 例**（对照池无限时 61,671 例）。
MVP 现有 5,364 例，**差 9.2 – 13.4 倍**。
作为参照：**史上最大的黑色素瘤 meta（Landi 2020）确诊病例总共 30,134 例，而且全部在排除名单里。**

### 4.1 三条路的处置（2026-08-21 更新）

| 路 | 状态 |
|---|---|
| ① MVP labelled pilot | **待决**——技术上今天就能跑，但有一个不可逆的代价，见 §4.1.1 |
| ② 退回方案作者 | ✅ **已做**：`S40_QUERY_TO_PROTOCOL_AUTHOR.md` |
| ③ 新建多生物库 meta | ❌ **已关闭**，见 §4.1.2 |

另：**§3 的前提三已部分完成**（`S40_LOCK.md`，49 个文件已散列，
且**先于**任何外部汇总统计），缺容器镜像与 `renv.lock`。

#### 4.1.1 路 ① 的不可逆代价

MVP 的全 P 值是公开的，跑一次不需要任何许可。**但"看过"是不可撤销的**：
一旦本项目打开过 MVP 的汇总统计，此后**任何**基于 MVP 的分析
——包括将来用 Wheless 组织学确诊口径新做的那个 GWAS，因为参与者是同一批人——
都不能再被描述为事前的。

⚠ 这正是 S40 §3、以及本项目 S33/S39 两次教训的同一件事。
**所以这一步不由本项目自行决定。**

#### 4.1.2 路 ③ 为什么关闭

要凑到 R = 4（N_eff = 197,346），按对照池 100 万反解：

| 组织学确诊侵袭性病例 | N_eff | R | g（f = 0.50）|
|---:|---:|---:|---:|
| 5,364（MVP 现有）| 21,342 | 0.43 | 0.329 |
| 15,000 | 59,113 | 1.20 | 0.547 |
| 25,000 | 97,561 | 1.98 | 0.703 |
| 35,000 | 135,266 | 2.74 | 0.828 |
| 50,000 | 190,476 | 3.86 | 0.982 |

**即使凑到 50,000 例，g 仍是 0.982，还差一点。** 要过线需要约 **52,000 例**。

**而 Landi 2020 的 confirmed 病例总共 30,134 例，并且全部在排除名单里。**
路 ③ 要求组建一个比史上最大黑色素瘤 GWAS 还大 1.7 倍的池子，
**且其中每一个队列都必须是当年没有参加 Landi、UKB、GERA、FinnGen 的**。
**这不是"资源不够"，这是算术上不成立。**

#### 4.1.3 路 ④：2025 年扩展 melanoma consortium 的 leave-overlap-out——也不够

⚠ **数字来源**：仓库里出现的 `4x2_EXTERNAL_COHORT_HANDOFF.md`（**非本次分析产出**）
引 ASHG 2025 摘要，称该 meta 为 **70,966 例 / 1,597,619 对照**。
**本项目未核实这两个数**，下面是**以它为真**时的条件计算。

要过 R = 4，new-only 病例需 **50,909 例**（对照 1,597,619）。而：

| 剔除到哪一步 | 剩余 new-only | N_eff | R | g（f = 0.50）| |
|---|---:|---:|---:|---:|---|
| **上界**：只剔 Landi 的 36,760 | 34,206 | 133,956 | 2.72 | 0.824 | ❌ |
| 再剔 FinnGen R12（5,753）| 28,453 | 111,821 | 2.27 | 0.753 | ❌ |
| 再剔 MVP phecode（10,468）| 17,985 | 71,139 | 1.44 | 0.600 | ❌ |
| 再剔 MVP 宽口径（45,274）| **0** | — | — | — | 空集 |

**第一行是上界**——它甚至没有剔除 §1.3 必须剔除的 FinnGen 与 MVP，就已经不够了。
**所以这条路的失败不依赖于我对该 meta 具体构成的不确定性。**

⚠ 但这条线索**仍然值得追**：若那两个数属实，它是目前唯一有可能靠"再等几年、再多几个队列"
达到门槛的路径——需要 new-only 从 34,206 涨到 50,909，约 1.5 倍。
**建议向联盟索取准确的 leave-overlap-out 病例构成，而不是采信摘要数字。**

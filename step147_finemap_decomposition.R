...
for (R0 in REGIONS) {
  ...
  for (src in names(srcs)) {
    ...
    cs_at <- function(L) {
      ...
      if (is.null(fit)) return(c(n = NA_integer_, hi = NA_integer_))
      cs <- tryCatch(susie_get_cs(fit, Xcorr = Rs), error = function(e) NULL)
      if (is.null(cs) || is.null(cs$cs)) return(c(n = 0L, hi = 0L))
      hi <- if (!is.null(cs$purity))
        sum(cs$purity$min.abs.corr >= 0.5, na.rm = TRUE) else NA_integer_
      c(n = length(cs$cs), hi = hi)
    }
    ...
    cat(sprintf("  %-8s n=%d | minP=%.2e | cs L=10: %s (hi %s), L=20: %s | s=%.3f | %s\n",
                src, nrow(d), min(d$pval, na.rm = TRUE),
                a10[["n"]], a10[["hi"]], a20[["n"]], s_est, v))
...
for (g in c(TRUE, FALSE)) {
  ...
  for (i in seq_len(nrow(s)))
    cat(sprintf("  %-9s minP %9.2e  cs %2s  s %.3f  %s\n", s$region[i],
                s$min_pval[i], s$n_cs_L10[i], s$estimate_s[i], s$verdict[i]))
}
cat("\nwrote 147a\n")

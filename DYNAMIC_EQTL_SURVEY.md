...
### ⚠ 由此暴露的空白（**本节初稿写错了，`step117` 扫描后据实更正**）

**初稿写的是**："目前没有大样本的多细胞因子极化条件 × eQTL 资源。"
**这句话是错的。** 对全部 466 个基因表达数据集（50 个研究）按三条判据筛选
（是 eQTL 资源 / 供体 ≥100 / 该研究含 >1 个非静息条件），**六个研究达标**：

| 研究 | 细胞 | 条件数 | 刺激臂最大 n |
|---|---|---|---|
| **MacroMap** | **巨噬细胞** | **24**（IFNb/IFNg/IL4/IL10/LPS/PIC/Pam3CSK4… × 6h/24h）| 199 |
| Quach_2016 | **单核细胞** | 4（流感 / LPS / Pam3CSK4 / R848，6h）| 198 |
| Nassiri_2025 | **单核细胞** | 2（IFNg / LPS 24h）| 176 |
| Nedelec_2016 | **巨噬细胞** | 2（李斯特菌 / 沙门菌 5h）| 167 |
| Jerber_2021 | 多巴胺能神经元 / 星形胶质 | 4 | 167 |
| Cytoimmgen_reannotated | CD4 T | 4 | 106 |

**更正后的正确陈述**（这一版更强，也更值得写进正文）：

> **多条件、足够功效的刺激 eQTL 资源是存在的——但全在髓系。**
> 巨噬细胞有 24 个条件（MacroMap），单核细胞有 4 个（Quach）。
> **而 CD4⁺ T 细胞只有一个多条件资源，就是 Cytoimmgen / Soskic，
> 也就是本文自己的暴露。** 其余带刺激的 CD4 资源都是**单一条件**：
> DICE（anti-CD3/CD28 4h）、Randolph（流感 6h）。

→ **本文所审计的这套框架（dynamic CD4 eQTL MR），
其"动态"这一维实际上只由一个资源支撑**，
而髓系免疫学在同一问题上有多个可互证的资源。
这是关于该领域数据结构的事实，直接强化本文关于暴露轴的论点。

⚠ 初稿那句错在把"CD4 上没有"推广成了"没有"。**扫描前不该写那句话。**
